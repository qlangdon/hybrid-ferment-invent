...
FDC1 = read.alignment("FDC1_AA_gapsRemovedTrimmed.fas", format = "FASTA", forceToLower = F)
HaploKey = read.delim("FDC1_haplotypeKey.txt", sep = "\t", header = F)
colnames(HaploKey) = c("Strain", "Origin", "Species", "Population", "Pop2")

FDC1_m = as.matrix.alignment(FDC1)
colnames(FDC1_m) = c(1:ncol(FDC1_m))

SameCount = matrix(0, nrow = 1, ncol = ncol(FDC1_m))
colnames(SameCount) = c(1:ncol(SameCount))
for(i in 1:ncol(FDC1_m)){
  SameCount[1,i] = length(which(unlist(FDC1_m[,i] == FDC1_m[1,i]) == TRUE))
}

VARIABLESITES = as.numeric(colnames(SameCount)[which(SameCount[1,] != max(SameCount[1,]))])

FDC1_var = FDC1_m[,VARIABLESITES]

#write.csv(FDC1_var, file = "FDC1_var.csv")

################## Find Haplotypes ######################
STNAMES = rownames(FDC1_var)
SITES = colnames(FDC1_var)
...
Rep_m = matrix(0, ncol = ncol(FDC1_var), nrow = 1)
#rownames(Rep_m) = Representatives$Group
colnames(Rep_m) = colnames(FDC1_var)
Rep_m = as.data.frame(Rep_m)

for(i in 1:nrow(Representatives)){
  TEMP = t(as.matrix(FDC1_var[which(rownames(FDC1_var) == Representatives[i,2]),]))
...
Haplotype_df$lty = rep(1, nrow(Haplotype_df))
Haplotype_df[which(Haplotype_df$Differences >= 100 ), "lty"] = 2
...
write.csv(HaploStrain, file = "HaploStrain_FDC1_AA_ungaps.csv")
...
coords <- norm_coords(coords, ymin=-30, ymax=30, xmin=-30, xmax=30)

HaploKey[which(HaploKey$Species == 0),"Species"] = 1

colors <- c("#0092FF", "#FF0000", "#525252", "#707070")
...
V(g)$pie.color = list(c("#CC79A7",
                        ...
                        "#F3F3F3",
                        "#AAAAAA"))
pdf("HaplotypeNetwork_Population_AA_FDC1_community_ungaps.pdf", height = 45, width = 45)
...
V(g)$pie.color = list(c("#cc79a7", "#0072b2", "#d52900"))
pdf("HaplotypeNetwork_Origin_AA_FDC1_community_ungaps_labe.pdf", height = 25, width = 25)
plot(c1,g,vertex.color=colors[membership(fc)],  
...
save(list = ls(),file = "FDC1_AA_Haplotype_community.RData")