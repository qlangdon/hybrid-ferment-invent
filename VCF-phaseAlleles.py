...
outFile = open(outName+"_phaseInfo.txt", 'w')
outFile.write("chr\tchrPos\tgenomePos\tfilteredDepth\thetCalled\trefAllele\trefType\trefFreq\tallele_1\ttype_1\tfreq_1\thapStart\thap1\thap2\n")
    ...
    if re.match('##contig', currentLine): #look of the line with chr length info
        ...
        if re.search(",",altAllele):
            ...
            if len(altAllele)==1:
                ...
                altTyDict[1] = "in"
        genotypeFields = SNPline[8].split(":")
        genotypeInfo = SNPline[9].split(":")
        if "DP" in genotypeFields:
            hetState = 'NA'
            ...
            if int(DP) > 3:
                ...
                if "HP" in genotypeFields:
                    haplotypeInfo = genotypeInfo[4].split(",")
                    hapStart = haplotypeInfo[0].split("-")[0]
                    firstHap = haplotypeInfo[0].split("-")[1]
                    secondHap = haplotypeInfo[1].split("-")[1]
                else:
                    hapStart = "NA"
                    firstHap = "NA"
                    secondHap = "NA"
                outFile.write(chr + "\t" + chrPos + "\t" + str(genomePos) + "\t" + str(DP) + "\t" + hetState + "\t" + refAllele + "\t" + refType + "\t" + refFreq)
                outFile.write("\t" + altAlDict[1] + "\t" + altTyDict[1] + "\t" + altDeDict[1]+ "\t" + str(hapStart)+ "\t" +str(firstHap)+ "\t" +str(secondHap))
                outFile.write("\n")
            else:
                outFile.write(chr + "\t" + chrPos + "\t" + str(genomePos) + "\t" + str(DP) + "\t" + hetState + "\t" + refAllele + "\t" + refType + "\tNA")
                outFile.write("\t" + altAlDict[1] + "\t" + altTyDict[1] + "\tNA\tNA\tNA\tNA")
                outFile.write("\n")
        else:
            outFile.write(chr + "\t" + chrPos + "\t" + str(genomePos) + "\t0\tNA\t" + refAllele + "\t" + refType + "\tNA")
            outFile.write("\t" + altAlDict[1] + "\t" + altTyDict[1] + "\tNA\tNA\tNA\tNA")
            outFile.write("\n")
